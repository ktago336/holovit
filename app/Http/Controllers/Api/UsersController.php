...
class UsersController extends Controller {
   ...
   public function dashboard() {
    ...
    public function myorders()
    {
        ...
        $userid = $tokenData['user_id'];
        $ordersData = Order::where('user_id',$userid)->where('status',1)->orderBy('created_at', 'DESC')->get();
    ...
    public function mywallet()
    {
        ...
        $userid = $tokenData['user_id'];
	$userInfo = User::where('id', $userid)->first();
        $walletsData = Wallet::where('user_id',$userid)->where('status',1)->where('source', '!=' ,'Pay via Paypal')->orderBy('id', 'DESC')->get();
        $data['response_data'] = array();
        $i = 0;
	$detailsarray = array();
    ...
    public function mypayments()
    {
        $tokenData = $this->requestAuthentication('GET');
        $userid = $tokenData['user_id'];
        $paymentsData = Payment::where('user_id',$userid)->orderBy('id', 'DESC')->get();
        $data['response_data'] = array();
        $i = 0;
        ...
        if(($userData['payment_type'] == 'Wallet') && ($userInfo->wallet_balance < $total_price))
	{
	    ...
	    $data['response_data'] = '';
            $data['response_msg'] = 'You have purchased this Deal successfully';
            $data['response_status'] = 'success';
            echo json_encode($data);
            exit;
    }
