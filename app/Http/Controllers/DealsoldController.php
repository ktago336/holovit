...
//        print_r($users);exit;
        if ($request->ajax()) {
            return view('elements.deals.index', ['allrecords' => $users]);
        }
        return view('deals.index', ['title' => $pageTitle, 'mydeals' => 'active', 'allrecords' => $users]);
    }
    ...
    public function add() {
        ...
        if (!empty($input)) {
            ...
            if ($validator->fails()) {
                return Redirect::to('/deals/add')->withErrors($validator)->withInput();
            } else {
        ...
        return view('deals.add', ['title' => $pageTitle, 'mydeals' => 'active', 'product_name' => $product_name]);
    }
...
//                echo "<pre>"; print_r($input);exit;
                ...
                return Redirect::to('deals/edit/'.$slug);
            }
        }
        return view('deals.edit', ['title' => $pageTitle, 'mydeals' => 'active', 'recordInfo' => $recordInfo, 'product_name' => $product_name]);
    }
    
    public function activate($slug=null){
        if($slug){
            Deal::where('slug', $slug)->update(array('status' => '1'));
            return view('elements.update_status', ['action'=>'deals/deactivate/' . $slug, 'status'=>1]);
        }
    }
    ...
    public function deactivate($slug=null){ 
        if($slug){
            ...
            return view('elements.update_status', ['action'=>'deals/activate/' . $slug, 'status'=>0]);
        }
    }
