...
//mail to merchant
                ...
                $toRepArray = array('[!email!]', '[!username!]' ,'[!link!]', '[!HTTP_PATH!]', '[!SITE_TITLE!]');
                $fromRepArray = array($emailId, $name, $link, HTTP_PATH, SITE_TITLE);
                $emailSubject = str_replace($toRepArray, $fromRepArray, $emailTemplate->subject);
                $emailBody = str_replace($toRepArray, $fromRepArray, $emailTemplate->template);
				Mail::to($emailId)->send(new SendMailable($emailBody,$emailSubject));
				
                Session::flash('success_message', "We will sent you an confirmation mail within some time. Please check your spam folder if you do not receive the email.");
...
//                print_r($input);
				...
				$updata['google_link'] = $input['google_link'];
