...
class Controller extends BaseController
{
    ...
    // /*Get Unique number*/
    // public function getUniqueModelId($tablename = 'appointments', $fieldname = 'unique_number', $number_of_digits = 8, $token_type = '0', $prefix = '') {
    //     if($token_type == '1'){
    ...
    //     //$slug = str_replace(' ', '-', strtolower($slug));
    //     $isSlugExist = DB::table($tablename)->where($fieldname,$token)->first();               
    //     if (!empty($isSlugExist)) {
    //         $token = $token.bin2hex(openssl_random_pseudo_bytes(6));
    //         $this->getUniqueModelId($tablename = 'appointments', $fieldname = 'appointment_number', $number_of_digits = 8, $token_type = '1', $prefix = 'ABS');
    //     }
    //     return $token;
    // }
    ...
    public function requestAuthentication($mth='GET', $checkToken=0) {
        ...
        //echo "<pre>";print_r($headers['HTTP_TOKEN']);exit;
        if ($apiKey != API_KEY) {
            echo $this->errorOutputResult('Unauthorized Access.');
            exit;
        }
     ...
     public function requestAuthenticationn($mth='GET', $checkToken=0) {
        $reqMethod = $_SERVER["REQUEST_METHOD"];
        ...
        if($checkToken == 1){
            ...
            $tokenData = $this->verifyTokenn($token);
            $tokenData = (array)$tokenData;
    ...
    public function verifyTokenn($token = null) {
        //$tokenData  = JWTAuth::toUser($token);
    //print_r($token);exit;
    $tokenData = Merchant::where('token',$token)->first();
        if (!empty($tokenData)) {
            $data = array();
            $data['user_id']  = $tokenData->id;
            $data['user_name']  = $tokenData->first_name.' '.$tokenData->last_name;
