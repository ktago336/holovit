...
//  echo "<pre>";      print_r($recordInfo);exit;
        ...
        if (!empty($input)) {
            ...
            $validator = Validator::make($input, $rules);
            if ($validator->fails()) {
                return Redirect::to('/admin/admins/why-buy')->withErrors($validator)->withInput();
            } else {
                if (Input::hasFile('step1_image')) {
                    $file_logo = Input::file('step1_image');
                    $uploadedFileName = $this->uploadImageWithSameName($file_logo, LOGO_IMAGE_UPLOAD_PATH);
                    $input['step1_image'] = $uploadedFileName;
                } else {
                    unset($input['step1_image']);
                }

                if (Input::hasFile('step2_image')) {
                    $file = Input::file('step2_image');
                    $uploadedFileName = $this->uploadImageWithSameName($file, LOGO_IMAGE_UPLOAD_PATH);
                    $input['step2_image'] = $uploadedFileName;
                } else {
                    unset($input['step2_image']);
                }

                if (Input::hasFile('step3_image')) {
                    $file = Input::file('step3_image');
                    $uploadedFileName = $this->uploadImageWithSameName($file, LOGO_IMAGE_UPLOAD_PATH);
                    $input['step3_image'] = $uploadedFileName;
                } else {
                    unset($input['step3_image']);
                }
...
//print_r($serialisedData);exit;
                DB::table('settings')->where('id', $recordInfo->id)->update($serialisedData);
                Session::flash('success_message', "Why Buy section updated successfully.");

                return Redirect::to('admin/admins/why-buy');
            }
        }
        
            return view('admin.admins.whyBuy', ['title' => $pageTitle, $activetab => 1, 'recordInfo' => $recordInfo]);
