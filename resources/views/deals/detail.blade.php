...
<section class="slider details">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12 col-md-7 col-sm-12 col-lg-7">
                            <div class="row">
								...
								<?php $imgcnt = $imgcnt+1; ?>
								<div class="col-md-6">
									{{HTML::image(MERCHANT_FULL_DISPLAY_PATH.$image, SITE_TITLE,['style'=>""])}}
                                </div>
                                @if($imgcnt == 2)
                                <?php break;?>
                                @endif
                                                    ...
                                                    <div class="margin-bottom-m">
                                                        <div class="row">
                                                            <div class="col-sm-8 col-md-8">
                                                                <h3 class="font-xxxl font-weight-bold txt-primary line-height-sm">{{$dealinfo->deal_name}} (30min)</h3>
                                                            ...
                                                            <div class="col-sm-4 col-md-4 txt-right">
                                                                <p>
                                                                    ...
                                                                    <i class="nb-icon nb-icon_rupee nb-icon--xs"></i>{{CURR.$dealinfo->voucher_price}}</span>
                                                                    <b class="font-xxxl font-weight-bold txt-primary margin-left-xs">
                                                                    <i class="nb-icon nb-icon_rupee nb-icon--md" content="USD" itemprop="priceCurrency"></i>{{CURR.$dealinfo->final_price}}</b>
                                                                </p>
                                                            ...
                                                            <div class="col-sm-7 col-md-8">
                                                                <div class="op-timings">
                                                                    <p class="op-timings__content">
                                                                        ...
                                                                        <span class="txt-primary font-weight-semibold font-sm">{{$dealinfo->valid_for}} Person</span>
                                                                    </p>
                                                                    <!--<p class="op-timings__content">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-5 col-md-4 txt-right">
                                                            <span class="qty-counter">
                                                                <button id="blank-{{$dealinfo->id}}" class="btn--brand qty-counter__btn-activate" onclick="addtobook({{$dealinfo->id}},{{$dealinfo->final_price}})">
                                                                <span class="full-width txt-center txt-white font-bold font-xxl">Add</span>
                                                                <span class="txt-white font-xxxl line-height-xs">+</span>
                                                                </button>
                                                                <button "add-{{$dealinfo->id}}" class="qty-counter__btn-increment" onclick="removetobook({{$dealinfo->id}},{{$dealinfo->final_price}})">–</button>
                                                                <input class="deal_qty" data-id="{{$dealinfo->id}}" id="deal_qty_{{$dealinfo->id}}" name="deal_qty" readonly="" type="text" value="0">
                                                                <button "remove-{{$dealinfo->id}}" class="qty-counter__btn-decrement" onclick="addtobook({{$dealinfo->id}},{{$dealinfo->final_price}})">+</button>
                                                            </span>
                                                            <span style="display:none;" id="deal-name-{{$dealinfo->id}}">{{\Illuminate\Support\Str::limit($dealinfo->deal_name,20, $end='...')}}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            </option-card>
                                        </div>
										<input type="hidden" id="discount-price-{{$dealinfo->id}}" name="discount_price" value="{{($dealinfo->voucher_price-$dealinfo->final_price)}}">
										@endforeach
										@endif
                                  </div>
                                   </div>
                            ...
                            <div class="row">
                                <div class="col-sm-8 col-md-7 col-lg-9">
                                    <div class="margin-bottom-s">
                                        <h3>
                                            <!----><p class="font-xxxl font-weight-bold txt-primary margin-bottom-xs">{{$recordInfo->currentDeal->deal_name}}</p>
                                            <!---->
											@if(count($recordInfo->allDeal) > 1)
												<p class="font-md font-weight-semibold line-height-default txt-tertiary">+ {{(count($recordInfo->allDeal)-1)}} more Deal</p>
											@endif
                                        </h3>
                                    </div>
                                    <!----><p class="font-lg line-height-xs">
                                        <span class="margin-right-xs">
                                            ...
                                            <span class="font-lg txt-primary font-weight-bold">{{CURR.$recordInfo->currentDeal->final_price}}</span>
                                        </span>
            ...
            <div class="margin-top-xl margin-bottom-l">
                <div class="margin-bottom-l margin-top-m">
                    <!----><h2 class="line-height-default font-weight-semibold margin-bottom-s txt-capitalize font-xxxl">About {{$recordInfo->busineess_name}}</h2>
                    <!----><p class="card-main__desc txt-secondary margin-bottom-m font-xl">{{$recordInfo->about_us}}</p>
                    <!---->
                </div>
            </div>
           <h2 class="line-height-default font-weight-semibold margin-bottom-s txt-capitalize font-xxxl"> Address</h2>
               <p class="card-main__desc txt-secondary margin-bottom-m font-xl">{{$recordInfo->address.', '.(isset($recordInfo->City->city_name)?$recordInfo->City->city_name.', ':'').$recordInfo->zipcode}}</p>
                <h2 class="line-height-default font-weight-semibold margin-bottom-s txt-capitalize font-xxxl"> Phone no.</h2>
               <p class="card-main__desc txt-secondary margin-bottom-m font-xl">{{$recordInfo->contact}}</p>
               ...
               <p class="card-main__desc txt-secondary margin-bottom-m font-xl">Cost for two - 2,600</p>
                                    ...
                                    <h2 class="line-height-default font-weight-semibold margin-bottom-s txt-capitalize font-xxxl">Follow {{$recordInfo->busineess_name}}</h2>
               <ul class="social_icon details">
					<?php $is_social_links = 0;?>
					@if(trim($recordInfo->facebook_link))
						<?php $is_social_links = 1;?>
					    <li class="facebook"><a href="{!! $recordInfo->facebook_link !!}" target="_blank"><i class="social-media fa fa-facebook" aria-hidden="true"></i></a></li>
					@endif
					@if(trim($recordInfo->instagram_link))
						<?php $is_social_links = 1;?>
                        <li class="instagram"><a href="{!! $recordInfo->instagram_link !!}" target="_blank"><i class="social-media fa fa-instagram" aria-hidden="true"></i></a></li>
					@endif
					@if(trim($recordInfo->linkedin_link))
						<?php $is_social_links = 1;?>
                        <li class="linkedin"><a href="{!! $recordInfo->linkedin_link !!}" target="_blank"><i class="social-media fa fa-linkedin" aria-hidden="true"></i></a></li>
					@endif
					@if(trim($recordInfo->twitter_link))
						<?php $is_social_links = 1;?>
                        <li class="twitter"><a href="{!! $recordInfo->twitter_link !!}" target="_blank"><i class="social-media fa fa-twitter" aria-hidden="true"></i></a></li>
					@endif
					@if(trim($recordInfo->google_link))
						<?php $is_social_links = 1;?>
                        <li class="google"><a href="{!! $recordInfo->google_link !!}" target="_blank"><i class="social-media fa fa-google" aria-hidden="true"></i></a></li>
					@endif
					@if(trim($recordInfo->youtube_link))
						<?php $is_social_links = 1;?>
                        <li class="youtube"><a href="{!! $recordInfo->youtube_link !!}" target="_blank"><i class="social-media fa fa-youtube" aria-hidden="true"></i></a></li>
					@endif
					@if($is_social_links == 0)
                ...
                <h2 class="line-height-default font-weight-semibold margin-bottom-s txt-capitalize font-xxxl">{{$recordInfo->busineess_name}} Timings</h2>
                    ...
                    <div class="col-md-6">
                     ...
                     <div class="col-md-6">
						 ...
						 @foreach($week_days as $wd_key=>$wd_val)
                            ...
                            <p>{{ date("h:i A",strtotime($start_time_arr[array_search ($wd_key,$working_days_arr)])) }} - {{ date("h:i A",strtotime($end_time_arr[array_search ($wd_key,$working_days_arr)])) }}</p>
							@else
                            <p>Closed</p>
							@endif
						@endforeach
                                ...
                                <div class="images_details">
									...
									@if(array_filter($images))
										@foreach($images as $image)
											...
											@endif	
										<div class="col-md-4">
											{{HTML::image(MERCHANT_SMALL_DISPLAY_PATH.$image, SITE_TITLE,['style'=>""])}}
										</div>
											@if($i%3 == 0)
												<?php $is_open = 0;?>
												</div>
