            ...
            <table class="table table-bordered table-striped table-condensed cf">
                <thead class="cf ddpagingshorting">
                    <tr>
                        ...
                        <th class="sorting_paging">@sortablelink('total_amount', 'Total Paid Amount')</th>
                        <th class="sorting_paging">@sortablelink('is_voucher_redeemed', 'Voucher Status')</th>
                        <th class="sorting_paging">@sortablelink('created_at', 'Order Date')</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td data-title="Merchant">{{'#'.$allrecord->order_number}}</td>
                        <td data-title="Customer Name">{{$allrecord->User->first_name.' '.$allrecord->User->last_name}}</td>
                        <td data-title="Merchant">{{isset($allrecord->Merchant->busineess_name)?$allrecord->Merchant->busineess_name:'N/A'}}</td>
                        <td data-title="Total Paid Amount">{{CURR }}{{$allrecord->total_price}}</td>
                        <td data-title="Voucher Status">{{$allrecord->is_voucher_redeemed?"Redeemed":"Pending"}}</td>
                        <td data-title="Order Date">{{$allrecord->created_at}}</td>
                        <td data-title="Action">
                            <div id="loderstatus{{$allrecord->id}}" class="right_action_lo">{{HTML::image("public/img/loading.gif", SITE_TITLE)}}</div>
                            <a href="{{ URL::to( 'admin/orders/'.$allrecord->slug)}}" title="View Detail" class="btn btn-danger btn-xs"><i class="fa fa-eye"></i></a>
