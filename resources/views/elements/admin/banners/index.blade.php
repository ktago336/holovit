                ...
                <table class="table table-bordered table-striped table-condensed cf">
                    <thead class="cf ddpagingshorting">
                        <tr>
                            ...
                            <th class="sorting_paging">@sortablelink('banner_url', 'Banner Link')</th>
                    ...
                    <tbody>
                        ...
                        <label>{{HTML::image(BANNER_SMALL_DISPLAY_PATH.$allrecord->banner_image, SITE_TITLE,['style'=>"max-width: 50px"])}}</label>
                        @else
                        {{HTML::image('public/img/noimage.png','our-clients',['style'=>"max-width: 50px"])}}
                        @endif</td>
                                <td data-title="Title">{{$allrecord->title}}</td>
                                <td data-title="Banner Link">{{$allrecord->banner_url}}</td>
                                ...
                                <td data-title="Action">
                                   ...
                                   <span class="right_acdc" id="status{{$allrecord->id}}">
                                        @if($allrecord->status == '1')
                                            <a href="{{ URL::to( 'admin/banners/deactivate/'.$allrecord->slug)}}" title="Deactivate" class="deactivate"><button class="btn btn-success btn-xs"><i class="fa fa-check"></i></button></a>
                                        @else
                                            <a href="{{ URL::to( 'admin/banners/activate/'.$allrecord->slug)}}" title="Activate" class="activate"><button class="btn btn-danger btn-xs"><i class="fa fa-ban"></i></button></a>
                                        @endif
                                    </span>
