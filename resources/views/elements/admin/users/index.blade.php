        ...
        <div class="tbl-resp-listing">
            <table class="table table-bordered table-striped table-condensed cf">
                <thead class="cf ddpagingshorting">
                    <tr>
                        <th style="width:5%">#</th>
                        <th class="sorting_paging">@sortablelink('user_name', 'Customer Name')</th>
                ...
                <tbody> <?php //echo"<pre>"; print_r($allrecords);exit; ?>
                    ...
                    <tr>
                        ...
                        <label>{{HTML::image(PROFILE_SMALL_DISPLAY_PATH.$allrecord->profile_image, SITE_TITLE,['style'=>"max-width: 50px"])}}</label>
                        @else
                        {{HTML::image('public/img/noimage.png','our-clients',['style'=>"max-width: 50px"])}}
                        @endif
                        {{$allrecord->first_name. ' ' . $allrecord->last_name}}</td>
                        ...
                        <?php } } else{?><a href="{{ URL::to( 'admin/users/edit/'.$allrecord->slug)}}" title="Edit" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
                        <?php } ?>
                        <?php $role = 3; 
                        if(isset($checkSubRols[3])){ if ($adminLId == 1 || in_array($role, $checkSubRols[3])) { ?>
                            ...
                            <?php } } else {?><a href="{{ URL::to( 'admin/users/delete/'.$allrecord->slug)}}" title="Delete" class="btn btn-danger btn-xs action-list delete-list" onclick="return confirm('Are you sure you want to delete this record?')"><i class="fa fa-trash-o"></i></a>
                        <?php } ?>
