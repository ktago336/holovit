...
<div class="admin_loader admin_loader_front" id="loaderID">{{HTML::image("public/img/website_load.svg", SITE_TITLE)}}</div>
                      ...
                      <div class="card-main1">
                       ...
                       <?php $count = 0; ?>
						...
						@if($category_merchants)
							...
							@if($ct_id && isset($categories[$ct_id]))
    							...
    							@if($ct_id == $business_type_id)
    								@if($subcategory_merchants)
    									@foreach($subcategory_merchants as $sct_id=>$sct_count)
    									<div id="collapsetwo" class="<?php echo $slug==$subcategory_arr[$sct_id]['slug']?"active":""; ?>" data-parent="#accordion" style="">
    										<div class="question_form_sec" id="question_form_sec">
    											<label class="subcat_check full-width deltimesub" style="">
    												<a href="<?php echo HTTP_PATH; ?>/deals/search/{{$subcategory_arr[$sct_id]['slug']}}" class="txt-primary padding-left-xs" id="show_sub_cat_products">
    												{{$subcategory_arr[$sct_id]['category_name']}}
    												<span class="count flt-right">{{$sct_count}}</span>
    											  </a>
    											</label>
    										 </div>  
    									</div>	
